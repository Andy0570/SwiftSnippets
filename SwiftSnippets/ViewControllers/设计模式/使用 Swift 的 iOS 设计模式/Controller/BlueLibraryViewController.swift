...
//  SwiftSnippets
//
//  Created by Qilin Hu on 2024/12/13.
//
...
class BlueLibraryViewController: UIViewController {
    // 使用无枚举值的枚举类型的优点是，它不会被意外地实例化，并只作为一个纯命名空间。
    ...
    override func viewDidLoad() {
        ...
        // 默认加载第一张专辑
        showDataFromAlbum(at: currentAlbumIndex)
    }
    ...
    // 从专辑数组中获取所需专辑数据
    private func showDataFromAlbum(at index: Int) {
...
extension BlueLibraryViewController: HorizontalScrollerViewDelegate {
    func horizontalScrollerView(_ horizontalScrollerView: HorizontalScrollerView, didSelectViewAt index: Int) {
        ...
        showDataFromAlbum(at: index) // 刷新 tableView
    }
}
