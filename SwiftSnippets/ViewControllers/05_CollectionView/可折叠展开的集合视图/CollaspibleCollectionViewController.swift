...
final class CollaspibleCollectionViewController: UIViewController {
    private let data: [Item] = {
        return [
            Item(title: "Programming Languages", subItems: [Item(title: "Swift", subItems: []),
    ...
    private lazy var dataSource: CollectionDataSource = {
        ...
        // 注册重用 cell：item cell
        let subItemsCellRegistration = UICollectionView.CellRegistration<UICollectionViewListCell, Item> {
            cell, _, model in
            var contentConfiguration = cell.defaultContentConfiguration()
            contentConfiguration.text = model.title
