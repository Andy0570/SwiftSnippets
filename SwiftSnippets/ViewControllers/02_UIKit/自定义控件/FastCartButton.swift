...
final class FastCartButton: UIControl {
    // MARK: - Properties
    ...
    private var touchState: TouchState = .idle {
        didSet {
            ...
            case .cancelled:
                performTouchCancelled(animated: true)
            }
        }
    }

    /// 如果用户点击一个控件，然后将他们的手指从该控件拖出一个相对较小的距离（ <= 大约 70 point）, UIControl
    ...
    private func performTouchDownAnimations() {
        ...
        lineWidthConstraintGroup = [
            topWhiteLine.widthAnchor.constraint(equalToConstant: 32.0 + CGFloat.random(in: -4.0...4.0)),
            middleWhiteLine.widthAnchor.constraint(equalToConstant: 32.0 + CGFloat.random(in: -4.0...4.0)),
            bottomWhiteLine.widthAnchor.constraint(equalToConstant: 32.0 + CGFloat.random(in: -4.0...4.0))
        ]
        NSLayoutConstraint.activate(lineWidthConstraintGroup)
        ...
        UIView.animate(withDuration: 0.8, delay: 0.0, usingSpringWithDamping: 1.0, initialSpringVelocity: 0.0, options: .allowUserInteraction) {
            self.layoutIfNeeded()
    ...
    private func performTouchUpAnimations() {
        UIView.animateKeyframes(withDuration: 0.8, delay: 0.0, options: [.allowUserInteraction, .calculationModeCubic, .beginFromCurrentState]) {
            ...
            // 2.一旦推车开始向前移动，我们就迅速将推车旋转回其静止位置，这给人的印象是推车在起飞时“砰砰”地回到它的4个轮子上。
            ...
            self.performTouchCancelled(animated: false)
        }
    }

    private func performTouchCancelled(animated: Bool) {
        NSLayoutConstraint.deactivate(lineWidthConstraintGroup)
        lineWidthConstraintGroup.removeAll()
        lineWidthConstraintGroup = [
            topWhiteLine.widthAnchor.constraint(equalToConstant: 24),
