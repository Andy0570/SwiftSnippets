...
//  SwiftSnippets
//
//  Created by huqilin on 2025/8/6.
//
...
/**
 使用 UIViewPropertyAnimator 在 Swift 中构建交互式动画
 
 参考：<https://www.swiftkickmobile.com/post/building-better-app-animations-swift-uiviewpropertyanimator>
 */
class InteractiveAnimationViewController: UIViewController {
    ...
    private lazy var contentImageView: UIImageView = {
        ...
        imageView.image = UIImage(named: "animation_bg_normal")
    ...
    private lazy var openTitleLabel: UILabel = {
        ...
        // CGAffineTransform(scaleX: 0.65, y: 0.65): 缩放变换，x 和 y 坐标方向都缩小为原来的 65%。
        // CGAffineTransform(translationX: 0, y: -15): 平移变换，水平方向不动，y 方向向上移动 15 point。
    ...
    private lazy var reviewsImageView: UIImageView = {
        let imageView = UIImageView()
        imageView.translatesAutoresizingMaskIntoConstraints = false
        imageView.image = UIImage(named: "reviews_bg_normal")
    ...
    private func layout() {
        ...
        reviewsImageView.bottomAnchor.constraint(equalTo: popupView.safeAreaLayoutGuide.bottomAnchor).isActive = true
    ...
    private lazy var panRecognizer: InstantPanGestureRecognizer = {
        let recoginzer = InstantPanGestureRecognizer()
        recoginzer.addTarget(self, action: #selector(popupViewPanned(recognizer:)))
        return recoginzer
    }()
    ...
    private func animateTransitionIfNeeded(to state: State, duration: TimeInterval) {
        ...
        // 我们将 Animator 的 scrubsLinearly 属性设置为 false，这样动画的 fractionComplete 就会映射到缓入（ease-in）时间曲线，而不是线性时间曲线。
        // 通常跟随用户手指移动的动画应该采用线性时间曲线，因此该属性默认值为 true。
    ...
    @objc private func popupViewPanned(recognizer: UIPanGestureRecognizer) {
        switch recognizer.state {
            ...
            case .changed:
                ...
                // variable setup
                // 获取手指在 popupView 坐标系中，从手势开始到当前时刻的总拖动位移（translation）
                // translation.x: 水平方向拖动了多少点（>0 表示向右，<0 表示向左）
                // translation.y: 垂直方向拖动了多少点（>0 表示向下，<0 表示向上）
            ...
            case .ended:
                ...
                switch currentState {
                case .open:
                    if !shouldClose && !runningAnimators[0].isReversed { runningAnimators.forEach { $0.isReversed.toggle() } }
                    if shouldClose && runningAnimators[0].isReversed { runningAnimators.forEach { $0.isReversed.toggle() } }
                case .closed:
                    if shouldClose && !runningAnimators[0].isReversed { runningAnimators.forEach { $0.isReversed.toggle() } }
                    if !shouldClose && runningAnimators[0].isReversed { runningAnimators.forEach { $0.isReversed.toggle() } }
